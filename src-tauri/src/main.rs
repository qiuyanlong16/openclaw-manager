...
// Run a blocking Command with timeout using spawn_blocking + tokio::time::timeout.
// Returns Ok(output) on success, Err on timeout or other failure.
async fn run_with_timeout(mut cmd: std::process::Command) -> Result<std::process::Output, String> {
...
async fn check_version(cmd: &str, arg: &str) -> EnvStatus {
    let mut c = std::process::Command::new(cmd);
    c.arg(arg);
    match run_with_timeout(c).await {
...
async fn check_openclaw() -> EnvStatus {
...
#[tauri::command(async)]
async fn check_environment() -> EnvCheckResult {
    EnvCheckResult {
        node: check_version("node", "--version").await,
        git: check_version("git", "--version").await,
        openclaw: check_openclaw().await,
    }
}

#[tauri::command(async)]
async fn is_openclaw_installed() -> bool {
    check_openclaw().await.ok
}
...
async fn deploy_openclaw(app: tauri::AppHandle) -> Result<DeployResult, String> {
    ...
    let node = check_version("node", "--version").await;
    if node.ok {
    ...
    let git = check_version("git", "--version").await;
    if git.ok {
...
#[tauri::command(async)]
async fn get_gateway_status() -> GatewayStatusResult {
    ...
    // Fallback: try openclaw gateway status command (with timeout)
    let openclaw_bin = match get_openclaw_bin_path() {
        Ok(p) => p,
        Err(_) => return GatewayStatusResult { running: false },
    };
    let mut c = Command::new(&openclaw_bin);
    c.args(["gateway", "status"]);
    let output = run_with_timeout(c).await;
...
async fn list_skills_inner() -> Result<Vec<SkillInfo>, String> {
    let bin = get_openclaw_bin()?;
    let mut c = Command::new(&bin);
    c.args(["skills", "list", "--json"]);
    let output = run_with_timeout(c)
        .await
...
async fn list_skills() -> Vec<SkillInfo> {
    list_skills_inner().await.unwrap_or_default()
}

async fn get_blocked_skills() -> Result<Vec<String>, String> {
    let bin = get_openclaw_bin()?;
    let mut c = Command::new(&bin);
    c.args(["config", "get", "skills.blockBundled"]);
    let output = run_with_timeout(c).await;
...
async fn set_skill_enabled(name: String, enabled: bool) -> Result<(), String> {
    ...
    let mut blocked = get_blocked_skills().await?;
    if enabled {
